...
class SAWB(nn.Module):
    ...
    def forward(self, x):
        x = self.saq1(self.bias1(self.bn1(self.conv1(x))))
        shortcut1 = x
        x = self.saq2(self.bias2(self.bn2(self.conv2(x))))
        x = self.saq3(self.bias3(self.bn3(self.conv3(x))))
        x = shortcut1 + x
        x = self.saqr1(self.biasr1(self.bnr1(x)))
        x = self.saq4(self.bias4(self.bn4(self.conv4(x))))
        x = self.saq5(self.bias5(self.bn5(self.conv5(x))))
        shortcut2 = x
        x = self.saq6(self.bias6(self.bn6(self.conv6(x))))
        x = self.saq7(self.bias7(self.bn7(self.conv7(x))))
        x = self.saq8(self.bias8(self.bn8(self.conv8(x))))
        x = shortcut2 + x
        x = self.saqr2(self.biasr2(self.bnr2(x)))
        # print(x.size())
        x = self.dropout(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
